...
2. **`domain/circuitSolver.ts`** (`solveCircuit`) — pure function, the actual physics engine. Takes abstract `CircuitEdge[]` (nodeA/nodeB/resistance/emf/closed), independent of grid/parts concepts, which is why it's tested directly with hand-built edges rather than through the UI. Algorithm: repeatedly apply series elimination (any degree-2 node — this alone can produce a self-loop edge when both far endpoints coincide, which is the correctly-solved terminal state for a simple closed loop) and parallel merging (two edges sharing a node pair, restricted to zero-EMF pairs — merging a battery-bearing edge into a "parallel" pair only happens as the same series-elimination self-loop case, never as a real Thévenin-parallel combine; see the `flip()` helper, which must recursively re-orient composite children or current signs come out flipped). Unreduced leftovers after the loop determine status: fully resolved self-loop(s) → `ok`/`short` (R=0 with nonzero EMF); a single dangling non-self-loop edge containing a battery → `open`; anything else containing a battery → `unsupported` (genuine multi-loop/Kirchhoff topology, out of scope by design).
   **`ComponentResult.current` is signed relative to each part's own original `nodeA → nodeB`** (positive = terminal `a` → `b`), not relative to the loop's traversal direction. `solveTree` is handed a current measured along the traversal of whatever edge it's on, and series/parallel merging may have `flip()`-ed a leaf (swapping its nodeA/nodeB) to line it up with that traversal, so the leaf case sign-corrects by comparing against the original nodeA (`originalNodeA`). Getting this wrong once made every flow arrow (parts and wires) point the wrong way whenever a part was wired backwards relative to the battery, since arrows read the sign directly. Anything that *displays* current for a person (meters, bulb brightness, the inspector) must use `Math.abs`; only the flow arrows use the signed value.
...
`Palette.tsx` items start a drag on `onPointerDown` (with `touch-action: none` via the `touch-none` class) instead of `draggable`/`dragstart`/`dragover`/`drop` — HTML5 native DnD simply doesn't fire on touch browsers (tablets). `App.tsx` owns `draggingKind`, set by the palette's `onPointerDown` callback, plus `dragPointer` (the live pointer position, used to render a floating thumbnail that follows the finger/cursor — pointer-based dragging has no automatic drag-ghost the way native HTML5 DnD does, so without this a touch drag gives no visible feedback at all) and auto-scroll near the top/bottom viewport edge (the palette and canvas don't both fit on a narrow/tablet layout).

Despite `GridCanvas` already handling its own part/wire dragging via pointer events, the palette-drag-in flow (hover preview + drop) is **not** driven by `GridCanvas`'s own `onPointerMove`/`onPointerUp` — it's driven entirely from `App.tsx`'s `window`-level pointermove/pointerup/pointercancel listeners, which call an imperative handle `GridCanvas` exposes (`forwardRef`/`useImperativeHandle`, type `GridCanvasHandle`: `updateDragHover(clientX, clientY)`, `commitDrop(clientX, clientY, kind)`). This split exists because **a touch pointer's events stay targeted at whatever element received its `pointerdown` and only bubble from there** — they never actually fire *on* the canvas SVG during a drag that started on the palette, no matter where the finger physically moves, so `GridCanvas`'s own SVG-attached handlers would never see them. A `window` listener still receives them via bubbling regardless of that target-lock, which is why the pointer-tracking/ghost/auto-scroll (already window-driven) worked before this fix while the actual canvas drop (previously driven by `GridCanvas`'s own handlers) silently didn't. If you touch this flow, keep the "drop logic lives in `GridCanvas`, but is *invoked* from `App`'s window listener" split — moving it back onto the SVG's own pointer props will pass every desktop-mouse test while quietly breaking on touch again.
